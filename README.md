# Blooio Go API Library

<!-- x-release-please-start-version -->

<a href="https://pkg.go.dev/github.com/stainless-sdks/blooio-go"><img src="https://pkg.go.dev/badge/github.com/stainless-sdks/blooio-go.svg" alt="Go Reference"></a>
...
```go
import (
	"github.com/stainless-sdks/blooio-go" // imported as blooio
)
```
...
```sh
go get -u 'github.com/stainless-sdks/blooio-go@v0.0.1'
```
...
func main() {
	client := blooio.NewClient(
		option.WithAPIKey("My API Key"), // defaults to os.LookupEnv("BLOOIO_API_KEY")
	)
...
```go
p := blooio.ExampleParams{
	ID:   "id_xxx",             // required property
	Name: blooio.String("..."), // optional property

	Point: blooio.Point{
		X: 0,             // required field will serialize as 0
		Y: blooio.Int(1), // optional field will serialize as 1
		// ... omitted non-required fields will not be serialized
	},

	Origin: blooio.Origin{}, // the zero value of [Origin] is considered omitted
}
```
...
custom := param.Override[blooio.FooParams](12)
```
...
See the [full list of request options](https://pkg.go.dev/github.com/stainless-sdks/blooio-go/option).
...
`*blooio.Error`. This contains the `StatusCode`, `*http.Request`, and
...
client.Me.Get(context.TODO(), option.WithMaxRetries(5))
```
...
me, err := client.Me.Get(context.TODO(), option.WithResponseInto(&response))
if err != nil {
	// handle error
}
fmt.Printf("%+v\n", me)
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/blooio-go/issues) with questions, bugs, or suggestions.
